...
# firmware (boot.S + main.c packed into fw.mem). The vendored core is tracked;
# fw.mem is generated by build_fw.py at build time.
...
# freshness gate. All entries are mandatory build inputs: a missing one fails the
# freshness check (see _check_bitstream_freshness) rather than being skipped.
...
def _check_bitstream_freshness() -> str | None:
    """Return an error message if the bitfile is missing/stale vs its sources.

    A build input that is *missing* is a hard failure, not a skip: the vendored
    core, shared CPU/firmware sources, and the generated fw.mem are mandatory, so
    a bitstream whose inputs are unavailable cannot be certified fresh.
    """
