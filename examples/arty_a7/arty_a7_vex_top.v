...
module arty_a7_vex_top (
    ...
    vex_sys u_vex_sys (
        ...
        // Shared bus master -> test slave + monitor tap.  vex_sys leaves the
        // master-side AXI4 qualifier outputs open internally, so they are not
        // ports here (unlike the block-design mb_sys_wrapper).
    ...
    // Now sees both CPU (VexRiscv data master) and host (EJTAG-AXI) transactions
