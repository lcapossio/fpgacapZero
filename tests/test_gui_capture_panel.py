...
class TestCapturePanelApplyHistory(unittest.TestCase):
    ...
    def test_build_capture_config_rejects_b_combine_without_dual_compare(self) -> None:
        p = CapturePanel()
        p.set_hw_probe_info(
            {
                "sample_width": 8,
                ...
                "num_channels": 1,
                "trig_stages": 2,
                "compare_caps": 0x2_01C3,
                "has_dual_compare": False,
            },
        )
        enable = p.findChild(QCheckBox, "fcapz_capture_seq_enable")
        self.assertIsNotNone(enable)
        ...
        table = p.findChild(QTableWidget, "fcapz_capture_seq_table")
        self.assertIsNotNone(table)
        cmp_b = table.cellWidget(0, 1)
        combine = table.cellWidget(0, 2)
        self.assertIsInstance(cmp_b, QSpinBox)
        self.assertIsInstance(combine, QComboBox)
        cmp_b.setValue(1)
        combine.setCurrentIndex(2)
        with self.assertRaisesRegex(ValueError, "DUAL_COMPARE=1"):
            p.build_capture_config()
