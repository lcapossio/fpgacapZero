...
foreach cfg $configs {
    ...
    puts $fp "  wire \[PW-1:0\] ba,bsp; wire \[SW-1:0\] bd; wire bts; wire bs;"
    ...
    puts $fp "  fcapz_ela #(.SAMPLE_W(SW),.DEPTH(D),.TRIG_STAGES($stages),.STOR_QUAL($sq)) u_i(.sample_clk(clk),.sample_rst(rst),.probe_in(ctr),.trigger_in(1'b0),.trigger_out(),.armed_out(),.jtag_clk(jc),.jtag_rst(jr),.jtag_wr_en(jwe),.jtag_rd_en(jre),.jtag_addr(ja),.jtag_wdata(jwd),.jtag_rdata(jrd),.burst_rd_addr(ba),.burst_rd_data(bd),.burst_rd_ts_data(),.burst_start(bs),.burst_timestamp(bts),.burst_start_ptr(bsp));"
    puts $fp "  jtag_burst_read #(.SAMPLE_W(SW),.DEPTH(D)) u_b(.arst(rst),.tck(t2k),.tdi(t2i),.tdo(t2o),.capture(t2c),.shift_en(t2s),.update(t2u),.sel(t2l),.mem_addr(ba),.sample_data(bd),.timestamp_data(1'b0),.burst_start(bs),.burst_timestamp(bts),.burst_ptr_in(bsp));"
    puts $fp "endmodule"
    close $fp
