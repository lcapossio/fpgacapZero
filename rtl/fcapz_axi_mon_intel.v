...
// fpgacapZero AXI Monitor wrapper for Intel / Altera.
//
...
// Intel/Altera sld_virtual_jtag TAP primitive. Drop it onto an AXI4-Lite
// interface to capture and trigger on bus traffic over JTAG -- a portable,
// vendor-agnostic AXI bus monitor. Identical to fcapz_axi_mon_xilinx7 except
// the vendor TAP primitive (jtag_tap_intel instead of jtag_tap_xilinx7); the
// core, register pipe and burst path are shared. See docs/specs/axi_monitor.md.
//
// CTRL_CHAIN is the sld_virtual_jtag instance index (the Intel wrapper's CHAIN
// parameter), not a zero-based Python index -- pick one free of the other cores
// on the device (ELA control/data, EIO, and the EJTAG bridge each use their own).
//
// Usage:
//   fcapz_axi_mon_intel #(.ADDR_W(32), .DATA_W(32), .DEPTH(1024), .CTRL_CHAIN(5)) u_mon (
...
module fcapz_axi_mon_intel #(
    ...
    parameter CTRL_CHAIN   = 1,    // sld_virtual_jtag instance for control + burst
