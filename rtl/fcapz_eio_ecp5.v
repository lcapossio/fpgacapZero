...
// fpgacapZero EIO wrapper for Lattice ECP5 (standalone).
//
// WARNING: ECP5 has only 2 user chains (ER1 + ER2).  This standalone
// EIO wrapper uses one chain and CANNOT coexist with fcapz_ela_ecp5
// (which needs both chains).  To use ELA + EIO together on ECP5, set
// EIO_EN=1 on the fcapz_ela_ecp5 wrapper instead.
//
// This wrapper is suitable for EIO-only designs (no ELA).
//
// Usage:
//   fcapz_eio_ecp5 #(.IN_W(32), .OUT_W(32)) u_eio (
//       .probe_in(fabric_signals), .probe_out(driven_signals)
//   );

module fcapz_eio_ecp5 #(
    parameter IN_W  = 32,
    parameter OUT_W = 32,
    ...
    // ---- TAP wrapper ----
    jtag_tap_ecp5 #(.CHAIN(CHAIN)) u_tap (
