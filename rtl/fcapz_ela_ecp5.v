...
// fpgacapZero ELA wrapper for Lattice ECP5.
//
// ECP5 provides 2 user chains via JTAGG (ER1 + ER2):
//   ER1 (CHAIN=1) — register interface (control + readback)
//   ER2 (CHAIN=2) — burst data readout
//
// Optional EIO: set EIO_EN=1 to include an Embedded I/O core on the
// same register bus (ER1) via address mux.  EIO registers appear at
// offset 0x8000 from the host's perspective.  Standalone fcapz_eio_ecp5
// cannot coexist with this wrapper (only one JTAGG primitive allowed).
//
// Usage:
//   // ELA only
//   fcapz_ela_ecp5 #(.SAMPLE_W(8), .DEPTH(1024)) u_ela (
//       .sample_clk(clk), .sample_rst(rst), .probe_in(signals)
//   );
//
//   // ELA + EIO combined
//   fcapz_ela_ecp5 #(.SAMPLE_W(8), .DEPTH(1024),
...
module fcapz_ela_ecp5 #(
    ...
    jtag_tap_ecp5 #(.CHAIN(DATA_CHAIN)) u_tap_data (
