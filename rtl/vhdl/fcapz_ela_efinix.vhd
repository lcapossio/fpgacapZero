...
-- fpgacapZero ELA wrapper for Efinix Trion / Titanium (VHDL parity of
-- fcapz_ela_efinix.v). The Efinix JTAG User TAP is configured in the Efinity
-- Interface Designer and reaches the fabric as top-level ports, so this
-- wrapper exposes the two JTAG User TAP port groups (jtag1_* control,
-- jtag2_* burst) rather than hiding a primitive. Wire each group to one
-- Interface Designer JTAG User TAP block (a Trion device has two: USER1,
-- USER2). See fcapz_ela_efinix.v for usage and jtag_tap_efinix.vhd for the
-- signal mapping.
...
entity fcapz_ela_efinix is
    ...
    port (
        ...
        -- Efinix JTAG User TAP #1 (control chain)
        jtag1_tck     : in  std_logic;
        jtag1_drck    : in  std_logic;
        jtag1_tdi     : in  std_logic;
        jtag1_tdo     : out std_logic;
        jtag1_capture : in  std_logic;
        jtag1_shift   : in  std_logic;
        jtag1_update  : in  std_logic;
        jtag1_sel     : in  std_logic;

        -- Efinix JTAG User TAP #2 (burst-data chain)
        jtag2_tck     : in  std_logic;
        jtag2_drck    : in  std_logic;
        jtag2_tdi     : in  std_logic;
        jtag2_tdo     : out std_logic;
        jtag2_capture : in  std_logic;
        jtag2_shift   : in  std_logic;
        jtag2_update  : in  std_logic;
        jtag2_sel     : in  std_logic
    );
end entity fcapz_ela_efinix;

architecture rtl of fcapz_ela_efinix is
...
begin
    u_tap_ctrl : entity work.jtag_tap_efinix
        port map (
    ...
    u_tap_data : entity work.jtag_tap_efinix
        port map (
