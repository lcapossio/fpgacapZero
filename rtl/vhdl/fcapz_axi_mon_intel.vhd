...
-- fpgacapZero AXI Monitor wrapper for Intel / Altera (VHDL).
-- Native-VHDL translation of rtl/fcapz_axi_mon_intel.v.
--
...
-- Intel/Altera sld_virtual_jtag TAP primitive. Identical to the Xilinx-7
-- wrapper except the vendor TAP (jtag_tap_intel instead of jtag_tap_xilinx7).
-- CTRL_CHAIN is the sld_virtual_jtag instance index. See
-- docs/specs/axi_monitor.md.
...
entity fcapz_axi_mon_intel is
    generic (
        ...
        CTRL_CHAIN    : positive := 1     -- sld_virtual_jtag instance for control + burst
    );
    port (
...
end entity fcapz_axi_mon_intel;

architecture rtl of fcapz_axi_mon_intel is
...
begin
    ...
    u_tap_ctrl : entity work.jtag_tap_intel
        generic map (
            CHAIN => CTRL_CHAIN
        )
        port map (
